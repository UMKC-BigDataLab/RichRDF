...
def calSimilarity(subject, object, writer, sub1, sub2, context):
    ...
    try:
        ...
        o = "/".join(sub2.split('/')[:-1])
        writer.write(
            '<{}> <{}{}> <{}> <{}> .'.format(s + '/' +subject.capitalize(), relationToUse, relation, o +'/' + object.capitalize(),
                                             context))
        writer.write("\n")
        ...
        try:
            ...
            writer.write('\n')
            writer.write('_:{} <{}Semantic_Similarity> "{}" <{}> .'.format(hashed_output, relationToUse, str(Similarity),context))
            writer.write('\n')
